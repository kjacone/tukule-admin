...
@Component({
  ...
  styleUrl: './newdriver.component.scss'
})
export class NewdriverComponent implements OnInit {
  ...
  id: any;
  constructor(
    private route: ActivatedRoute, private firebase: AngularFireStorage,
  ...
  error(message) {
   this.comm.openSnackBar('Error: '+message);
    };
  
  success(message) {
    this.comm.openSnackBar('Success: '+message);
  }
  ...
  preview_banner(files:any) {
    ...
    if (this.banner_to_upload) {
      ...
      const storageRef = this.firebase.ref('drivers' + '/' + file1.name);
  ...
  create():any {
    ...
    if (format.test(this.fullname)) {
      this.comm.openSnackBar('Please enter valid name');
      return false;
    }
  ...
  update() :any {
    ...
    this.api.updateProfile(this.id, parma).then((data) => {
      this.loading = false;
      this.error('Success Driver updated');
      this.navCtrl.back();
    }, error => {
      console.log(error);
      this.loading = false;
