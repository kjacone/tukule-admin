...
import {
  ...
  Location as AutoCompleteLocation,
  GermanAddress,
...
export class RestdetailsComponent implements OnInit {
  ...
  placesRef: MatGoogleMapsAutocompleteDirective;
  dropdownList = [];
  selectedItems = [];
  dropdownSettings = {};
  ...
  getVenue() {
    this.api.getProfile(this.id).subscribe((data: any) => {
    ...
    this.api.getRestReview(this.id).subscribe((data) => {
      console.log(data);
      if (data && data.length) {
        this.reviews = data;
      }
